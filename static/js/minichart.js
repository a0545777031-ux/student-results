/* minichart.js v2 — self-contained, dependency-free charting with a modern look.
   Chart.js-like API: new Chart(canvas,{type,data,options}); supports
   bar (grouped + horizontal), line, doughnut, radar. Gradients, rounded marks,
   soft shadows, value labels, and a smooth entrance animation. No CDN needed. */
(function(){
  ...
  function fitText(ctx,txt,maxw){ if(ctx.measureText(txt).width<=maxw) return txt;
    let s=txt; while(s.length>1 && ctx.measureText(s+"…").width>maxw) s=s.slice(0,-1); return s+"…"; }
  ...
  Chart.prototype._bars=function(ctx,w,h,data,legend,horiz,prog){
    ...
    // soft grid
    ctx.strokeStyle="#eef2f0"; ctx.lineWidth=1;
    ctx.fillStyle="#9aa5a0"; ctx.font="10px "+FONT;
    const steps=4;
    if(!horiz){
    ...
    const barW=Math.min(span/nb*(horiz?0.9:0.72), horiz?24:52);
    // horizontal layout: reserve a name gutter (full names) + a value margin so scores never clip
    let hTL, hTR, hTW, hNameW=0;
    if(horiz){
      const RTL=isRTL(); const valW=56;
      ctx.font="600 11px "+FONT;
      hNameW=Math.min(Math.max(8, ...labels.map(l=>ctx.measureText(String(l)).width))+16, cw*0.46);
      if(RTL){ hTR=padL+cw-hNameW; hTL=padL+valW; }
      else   { hTL=padL+hNameW;    hTR=padL+cw-valW; }
      hTW=Math.max(30, hTR-hTL);
    }
    ...
    labels.forEach((lab,li)=>{
      dss.forEach((d,di)=>{
        ...
        if(horiz){
          ...
          const full=hTW*(v/maxV); const len=full*prog;
          const x0=RTL? (hTR-len):hTL;
          ctx.save(); ctx.shadowColor=rgba(col,.22); ctx.shadowBlur=6; ctx.shadowOffsetY=2;
          ctx.fillStyle=hgrad(ctx, RTL?hTR-full:hTL, full||1, col);
          roundRect(ctx,x0,yc,len,bth,6); ctx.fill(); ctx.restore();
          if(prog>0.9){
            const midY=yc+bth/2; ctx.textBaseline="middle";
            // score at the tip of the bar (kept inside a reserved margin so it never clips)
            ctx.fillStyle="#33413c"; ctx.font="700 11px "+FONT; ctx.textAlign=RTL?"right":"left";
            ctx.fillText(fmt(v), RTL?(hTR-full-7):(hTL+full+7), midY);
            // full student name in the gutter
            ctx.fillStyle="#2a3833"; ctx.font="600 11px "+FONT; ctx.textAlign=RTL?"right":"left";
            ctx.fillText(fitText(ctx,String(lab),hNameW-10), RTL?(padL+cw-2):(padL+2), midY);
          }
        } else {
  ...
  Chart.prototype._pie=function(ctx,w,h,data,legend,prog){
    ...
    // show only non-empty categories in the legend, but keep all for slices
    const shown=labels.map((l,i)=>({label:`${l} (${vals[i]})`,color:cols[i%cols.length],v:vals[i]}))
                      .filter(it=>it.v>0);
    const rows = shown.length>3?2:1; const legH = legend? (rows*20+10):8;
    const cx=w/2, cy=(h-legH)/2+6, R=Math.min(cx,cy)-8, r=R*0.60;
    const gap=0.025;
    let a=-Math.PI/2;
    vals.forEach((v,i)=>{
      if(v<=0) return;                                   // skip empty slices entirely
      const ang=(v/total)*Math.PI*2*prog;
      const col=cols[i%cols.length];
      ctx.beginPath(); ctx.moveTo(cx,cy);
      ctx.arc(cx,cy,R, a+gap, a+ang-gap); ctx.closePath();
      ctx.fillStyle=col; ctx.fill();                     // clean solid colour
      a+=ang;
    });
    // donut hole + soft inner ring
